...
- `slk-state register-role`, `replace-role`, `revise-plan`, `write`, `register-evidence`, and `handoff` are authenticated mutations. The credential is supplied only through `SLK_ROLE_CREDENTIAL`.
