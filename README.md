...
Version `1.8.3` makes the combined-role boundary executable: Checker never waits
for its own Supervisor responsibility, only Supervisor responsibility may seek
minimal Owner assistance, and Supervisor safeguard patrol is the final progress
guarantee. Version `1.8.2` added the pre-authorized Worker execution gate.
