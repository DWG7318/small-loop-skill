...
5. 由 Checker 给出 D1 结果；只有 PASS 或 FAIL 闭合 D1。检查工具或环境故障先定位，不直接算作产品缺陷；关键验收条件有可追溯直接证据才写 PASS，证据不足时记录 INCOMPLETE（不写为 PASS）并保持 D1 未闭合，优先换用现有可行验证方式，确需改变方案时交 Supervisor 协助：
   - `D1 PASS：CELL n/N`
...
- D1 INCOMPLETE 或需要改变 Run 方案时，Checker 保留责任并请 Supervisor 使用 `$slk-adjust-run` 协助取得证据或调整路线；Supervisor 提供新证据后仍由 Checker 重判 D1。
