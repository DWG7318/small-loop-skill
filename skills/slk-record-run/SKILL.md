...
通过 Grill 后，Supervisor 使用 `slk-state init-run` 写入 Run 定义、计划、方法版本、身份和端点，再使用 `$slk-manage-team` 建立后续成员。即使成员在其他 worktree 施工，三个角色仍指向同一中央状态；Checker 在 D1 前使用原始 CELL 定义和候选，依角色分区延后读取 Worker 的 D0、判断与建议，独立 D1 判断后再核对；D2 也先交原始 Run 目标、最终候选与端到端入口，详细历史随后核对。
