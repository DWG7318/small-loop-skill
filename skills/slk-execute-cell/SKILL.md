...
1. 重新确认 `CELL n/N`、目标、范围、D1 验收目标和当前候选基线；接收回执不结束当前 CELL 施工。
...
- 不要以“已接收”、单条命令结束或中间结果代替完整 CELL 候选交付，也不要自行进入未派发 CELL；不要提前把完整 D1/D2 当成 D0，也不要把 D0 结论或判断过程塞入初始 D1 交付；不要在交付后等待或读取 Checker 检查过程。
