...
6. 作为最后一项，通过 `Worker → Checker` 把候选作为下一编号的 `SLK TOKEN` 发送给 Checker，使用类似格式：`已完成，请检验：CELL n/N`。采用能够真实激活目标对话的操作，发送后结束本轮Worker工作，不使用`wait_threads`也不读取Checker状态；令牌到达即开始 D1，不增加接收回执轮次。

## Checker 未被激活时

在 Codex Desktop 中，使用 `send_message_to_thread` 这类会继续目标对话的真实激活操作，而不是只把文字写入后台聊天记录。Worker 发送后结束当前活动，不继续停留或读取 Checker 状态；平台明确返回消息未创建、目标不可用或投递失败时，调用 `$slk-recover-communication`，通过 Supervisor 优先恢复原 Checker。
